...
// можно было сделать так export function classNames(cls: string, mods?: Mods, additional?: string[]): string 
...
// filter(([key, value]) => Boolean(true)) -> оставляем только те элементы которые равны true, например hovered: true
// и тут нас интересуют значения (value)

// .map(([key, value]) => key) => тут нас интересует ключ и мы этот ключ возвращаем (key), ключи это название классов
